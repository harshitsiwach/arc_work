...
export function SmoothCursor({
  ...
  useEffect(() => {
    const checkEnabled = () => {
      ...
      return mediaQuery.matches && widthQuery.matches;
    };

    const onQueryChange = () => {
      const next = checkEnabled();
      setIsEnabled(next);
      if (!next) {
        setIsVisible(false);
    ...
    widthQuery.addEventListener("change", onQueryChange);

    return () => {
      mediaQuery.removeEventListener("change", onQueryChange);
      widthQuery.removeEventListener("change", onQueryChange);
      document.body.classList.remove("smooth-cursor-active");
    };
  }, []);
